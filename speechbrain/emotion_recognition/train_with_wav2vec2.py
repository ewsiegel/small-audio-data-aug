#!/usr/bin/env python3
"""Recipe for training an emotion recognition system from speech data only using a dataset.
...
class EmoIdBrain(sb.Brain):
    ...
    def on_stage_end(self, stage, stage_loss, epoch=None):
        ...
        if stage == sb.Stage.TEST:
            if hasattr(self.hparams, 'test_logger'):
                self.hparams.test_logger.log_stats(
                    {"Epoch loaded": 0},
                    test_stats=stats,
                )
            else:
